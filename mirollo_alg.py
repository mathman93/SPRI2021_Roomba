...
while True:
    try:
        ...
        # Update phase value based on sync algorithm.
            scaled_phase = 1 / current_phase # scale 360 down for function that is in terms of max of 1

            f = (1 / 2) * math.log(1 + (e ^ 2 - 1) * scaled_phase) + .1 # map state value and increment by small amount

            if f > 1: # if phase value with increment hits threshold, fire pulse. Must reset phase to 0
                phase_time -= time_to_take  # do we need to calculate how much time has passed?

            g = (exp(2 * f) - 1) / (exp(2) - 1) # map f back into phase

            current_phase = g * 360
            phase_time = phase_time - () # how much time do we take
